...
public class ProdutoService {
	...
	public void makeForm(int tipo, Produto produto, int orderBy) {
		...
		try{
			...
			form = new String(file.readAllBytes());
			file.close();
		...
		if(tipo == FORM_INSERT || tipo == FORM_UPDATE) {
			...
			umProduto += "\t\t</tr>";
			umProduto += "\t\t<tr>";
			umProduto += "\t\t\t<td>&nbsp;Descrição: <input required class=\"input--register\" type=\"text\" name=\"descricao\" placeholder=\""+ descricao +"\"></td>";
			umProduto += "\t\t\t<td>Preco: <input required class=\"input--register\" type=\"number\" step=\"0.01\" min=\"0\" name=\"preco\" value=\""+ produto.getPreco() +"\"></td>";
			umProduto += "\t\t\t<td>Quantidade: <input required class=\"input--register\" step=\"1\" type=\"number\" min=\"0\" name=\"quantidade\" value=\""+ produto.getQuantidade() +"\"></td>";
			umProduto += "\t\t</tr>";
			umProduto += "\t\t<tr>";
			umProduto += "\t\t\t<td>&nbsp;Data de fabricação: <input class=\"input--register\" type=\"datetime-local\" name=\"dataFabricacao\" value=\""+ produto.getDataFabricacao().toString() + "\"></td>";
			umProduto += "\t\t\t<td>Data de validade: <input class=\"input--register\" type=\"date\" name=\"dataValidade\" value=\""+ produto.getDataValidade().toString() + "\"></td>";
